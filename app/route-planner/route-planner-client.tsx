...
export default function RoutePlannerClient() {
  ...
  const [dynamicTrafficLevel, setDynamicTrafficLevel] = useState<
    "low" | "medium" | "high" | "unknown"
  >("unknown")

  const handleRouteSelect = async (
    originLocation: LocationPrediction,
    destinationLocation: LocationPrediction
  ) => {
    try {
      setLoading(true)
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">
            Plan your route and check real-time congestion predictions
          </p>
        </div>
            ...
            {selectedRoute && origin && destination ? (
              <>
                ...
                <RouteDetails
                  ...
                  duration={selectedRoute.duration_minutes}
                  trafficDensity={
                    dynamicTrafficLevel !== "unknown"
                      ? dynamicTrafficLevel
                      : selectedRoute.traffic_density
                  }
